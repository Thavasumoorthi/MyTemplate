.PHONY: docs test agent-setup agent-resetdb agent-smoke agent-test tests coverage lint security qa
...
coverage:
	pytest qa/test_backend.py qa/test_ui.py \
		--cov=appname \
		--cov-report=term-missing \
		--cov-report=xml:reports/coverage.xml \
		--cov-report=html:coverage_report \
		--junitxml=reports/unit-test-report.xml \
		-v
...
security:
	bandit -r appname -f json -o reports/bandit-report.json || true

qa: tests coverage lint security
